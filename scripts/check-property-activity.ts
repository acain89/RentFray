...
async function main(): Promise<void> {
  ...
  if (!propertyId) {
    ...
    console.error(
      "Usage: npx tsx scripts/check-property-activity.ts <property-id>",
    );
    process.exitCode = 1;
    return;
  }
  ...
  console.log("");
  console.log("====================================================");
  console.log(" RENTFRAY PROPERTY — PRODUCTION ACTIVITY CHECK");
  ...
  console.log(
    `Property type:            ${property.propertyType ?? "NOT SET"}`,
  );
  console.log(`Account created:          ${formatDate(property.createdAt)}`);
  console.log(
    `Property last updated:    ${formatDate(property.updatedAt)}`,
  );
  ...
  if (property.settings) {
    console.log(
      `Rent due day:             ${property.settings.rentDueDay}`,
    );
    console.log(
      `Grace period:             ${property.settings.gracePeriodDays} day(s)`,
    );
    console.log(
      `Late fees enabled:        ${yesNo(
        property.settings.lateFeeEnabled,
      )}`,
    );
    ...
    console.log(
      `Setup complete:           ${yesNo(
        property.settings.setupComplete,
      )}`,
    );

    console.log(
      `Settings last updated:    ${formatDate(
        property.settings.updatedAt,
      )}`,
    );
  } else {
  ...
  console.log("----------------------------------------------------");
  console.log(
    `Stripe account exists:    ${yesNo(
      Boolean(property.stripeAccountId),
    )}`,
  );
  ...
  console.log(
    `Management users:         ${property._count.managementUsers}`,
  );
  ...
  console.log(
    `Unit recurring fees:      ${property._count.recurringFees}`,
  );
  ...
  if (property.tiers.length > 0) {
    ...
    for (const tier of property.tiers) {
      ...
      console.log(
        `Base rent:                ${formatMoney(tier.baseRentCents)}`,
      );
      console.log(
        `Configured unit count:    ${tier.unitCount}`,
      );
      console.log(
        `Active unit count:        ${tier.activeUnitCount}`,
      );
      console.log(`Due day:                  ${tier.rentDueDay}`);
      console.log(
        `Grace period:             ${tier.gracePeriodDays} day(s)`,
      );
      console.log(
        `Initial late fee:         ${formatMoney(
          tier.lateFeeInitialCents,
        )}`,
      );
      console.log(
        `Daily late fee:           ${formatMoney(
          tier.lateFeeDailyCents,
        )}`,
      );
      console.log(
        `Max daily late-fee days:  ${tier.maxLateFeeDays}`,
      );
      console.log(
        `Processing fee:           ${formatMoney(
          tier.processingFeeCents,
        )}`,
      );
      ...
      console.log(
        `Last updated:             ${formatDate(tier.updatedAt)}`,
      );
      console.log("----------------------------------------------------");
    }
  }
  ...
  if (property.units.length > 0) {
    ...
    for (const unit of property.units) {
      ...
      console.log(
        `Last updated:             ${formatDate(unit.updatedAt)}`,
      );
      console.log("----------------------------------------------------");
    }
  }
  ...
  if (property.tenantAssignments.length > 0) {
    ...
    for (const assignment of property.tenantAssignments) {
      ...
      console.log(
        `Current assignment:       ${yesNo(assignment.isCurrent)}`,
      );
      console.log(
  ...
  console.log("----------------------------------------------------");
  console.log(
    `Total payment records:    ${property._count.payments}`,
  );
  console.log(
    `Pending in latest 10:     ${pendingPayments.length}`,
  );
  console.log(`Paid in latest 10:        ${paidPayments.length}`);
  console.log(
    `Failed in latest 10:      ${failedPayments.length}`,
  );
  ...
  if (paidPayments.length > 0) {
    console.log("");
    console.log(
      `${property.name.toUpperCase()} HAS PROCESSED A SUCCESSFUL PAYMENT THROUGH RENTFRAY.`,
    );
  } else if (pendingPayments.length > 0) {
    console.log("");
    console.log(
      `${property.name.toUpperCase()} CURRENTLY HAS A PAYMENT PROCESSING THROUGH RENTFRAY.`,
    );
