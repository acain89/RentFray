...
setupCompleteAcknowledgedAt?: string | null;
};
  session: {
...
useEffect(() => {
  ...
  setShowSetupSuccess(
    !data?.property?.setupCompleteAcknowledgedAt
  );
}, [
  onboardingComplete,
  data?.property?.setupCompleteAcknowledgedAt,
...
async function acknowledgeSetupComplete(): Promise<void> {
  try {
    const response = await fetch("/api/manager/setup-complete", {
      method: "POST",
      credentials: "include",
    });
    ...
    if (!response.ok || !json?.ok || !json.setupCompleteAcknowledgedAt) {
      alert(json?.error || "Failed to open dashboard.");
      return;
    }
      ...
      return {
        ...
        property: {
          ...
          setupCompleteAcknowledgedAt:
            json.setupCompleteAcknowledgedAt ?? null,
        },
      };
    });
